namespace ChessLogic
{
    public class Pawn : Piece
    {
        ...
        {
            return Board.IsInside(pos) && board.IsEmaty(pos);
        }

        private bool CanCapture(Position pos, Board board)
        {
            if (!Board.IsInside(pos) || board.IsEmaty(pos))
            {
                return false;
            }
        ...
        private IEnumerable<Move> ForwardMoves(Position from, Board board)
        {
            ...
            if (CanMoveTo(oneMovePos, board))
            {
                ...
                Position twoForward = oneMovePos + forward;

                if (!HasMoved && CanMoveTo(twoForward, board))
                {
                    yield return new DoublePawn(from, twoForward);
                }
            }
        }
